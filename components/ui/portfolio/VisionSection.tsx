import { Building2Icon, GlobeIcon, UsersIcon, StarIcon } from "lucide-react";
...
export function VisionSectionComponent({ vision, locale }: VisionSectionComponentProps) {
  ...
  return (
    <section className="space-y-8">
      <SectionHeading
        title={vision.headline?.[lang] ?? (lang === "fr" ? "Ma vision" : "My Vision")}
        ...
        {vision.items.map((item, i) => {
          ...
          return (
            ...
            >
              ...
              <div>
                <h3 className="font-bold text-sm text-foreground mb-2">{item.title[lang]}</h3>
                <p className="text-sm text-muted-foreground leading-relaxed">{item.description[lang]}</p>
              </div>
