...
export default async function Strength({ strength, locale }: StrengthProps) {
  ...
  return (
    ...
    >
      ...
      <ScrollReveal delay={0.1}>
        <div className="relative">
          ...
          <div className="absolute left-0 right-0 top-5 flex h-0.75 overflow-hidden">
            {strength.strengths.slice(0, -1).map((item, index) => {
              const next = strength.strengths[index + 1];
              const average = (item.percentage + next.percentage) / 2;
              return (
                <div
                  key={`${item.id}-line`}
                  className="flex-1 transition-colors duration-700"
                  style={{ backgroundColor: getStrengthColor(average) }}
                />
              );
            })}
          </div>
          ...
          <div className="relative z-10 flex justify-between">
            {strength.strengths.map((item) => {
              const color = getStrengthColor(item.percentage);
              return (
                <div key={item.id} className="flex flex-col items-center gap-4">
                  <div
                    className="flex h-10 w-10 items-center justify-center rounded-full ring-4 ring-background transition-colors duration-700"
      ...
      {/* Detail section — affiché si pas de modal */}
      {strength.detail && !example && (
              ...
              <div className="mt-6 grid gap-3 sm:grid-cols-2">
                {strength.detail.categories.map((cat) => (
                  <div key={cat.id} className="rounded-lg border border-border bg-secondary/50 p-3">
                    <h4 className="mb-1 text-sm font-semibold text-foreground">{t(cat.title)}</h4>
                    <p className="text-xs text-muted-foreground">{t(cat.description)}</p>
                  </div>
                ))}
              </div>
            )}
