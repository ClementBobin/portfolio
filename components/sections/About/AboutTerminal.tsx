...
function l(val: unknown, locale: string): string {
  ...
  // flat LocalizedString: { en: "foo", fr: "bar" }
  const direct = map[locale] ?? map["en"] ?? map["fr"];
  if (typeof direct === "string") return direct;

  // one level deeper (e.g. { libelle: { en, fr } })
...
function buildCommands(
  ...
  const { personal, contact, skills, experiences, education, projects, hobbies } = portfolio;
  ...
  return {
    ...
    bio: () => [
      ...
      ...(personal.status ? [`  Status    : ${l(personal.status, locale)}`] : []),
      "",
    ...
    contact: () => [
      ...
      ...contact
        .filter((c) => c.label)
        .map((c) => `  ${c.type.padEnd(10)}: ${c.label}${c.href ? `  →  ${c.href}` : ""}`),
    ],
    ...
    education: () => {
      ...
      for (const edu of education) {
        ...
        if (edu.specialty) lines.push(`    ${l(edu.specialty, locale)}`);
        lines.push("");
      }
      return lines;
    },
    ...
    hobbies: () => {
      ...
      for (const hobby of hobbies) {
        const details = hobby.details.map((d) => l(d, locale)).filter(Boolean);
...
export function AboutTerminal({ portfolio, locale }: AboutTerminalProps) {
  const { personal } = portfolio;
  const shellPrompt = `${personal.name.toLowerCase().replace(/\s+/g, "")}@portfolio:~`;

  // No useTranslations — all resolution done via l() to avoid overload issues
  const commands = buildCommands(portfolio, locale);

  const [lines, setLines] = useState<Line[]>([
    { text: "Type 'help' for available commands.", kind: "system" },
    { text: "", kind: "system" },
  ]);
  ...
  const run = (raw: string) => {
    ...
    const response: Line[] = handler
      ? handler().map((text) => ({ text, kind: "output" as const }))
      : [{ text: `command not found: '${cmd}'. Type 'help'.`, kind: "error" as const }];
  ...
  return (
    ...
    >
      ...
      {/* Output */}
      <div ref={outputRef} className="flex-1 overflow-y-auto px-4 py-3 leading-relaxed">
        {lines.map((line, i) => (
          <div key={i} className="whitespace-pre-wrap">
            {line.kind === "input"  && <span className="font-bold text-violet-400">{line.text}</span>}
            {line.kind === "error"  && <span className="text-red-400">{line.text}</span>}
            {line.kind === "system" && <span className="text-muted-foreground">{line.text}</span>}
            {line.kind === "output" && <span className="text-foreground/80">{line.text}</span>}
          </div>
        ))}
      </div>
